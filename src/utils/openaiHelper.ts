...
import { fetchContextMemory } from "../services/fetchContextMemory";
import { getMessageMemory } from "../services/getMessageMemory";
...
import QuoteLog, { IQuoteLog } from '../models/quoteLog.model';

// --- Custom Error Classes ---
class AppError extends Error {
  public readonly context?: Record<string, any>;
  constructor(message: string, public readonly cause?: Error, context?: Record<string, any>) {
    super(message); this.name = this.constructor.name; this.context = context;
...
// --- Types ---
interface Parameters { modelName?: string; temperature?: number; }
...
interface BusinessProfile {
  ...
  // Add your actual Cloudinary cloud name here if you have one configured
  // This is used in the refined prompt example
  cloudinaryCloudName?: string;
}
interface AiAgentProfile { name?: string; }

interface QuoteLogInputData {
  customerId: string; customerName: string; businessId: string;
  quoteTitle?: string; lineItems: LineItem[]; grandTotal: number;
  currency?: string; pdfLocalPath?: string; pdfCloudinaryUrl?: string;
  status: IQuoteLog['status'] | 'failed_email_bad_url'; errorMessage?: string; operationId: string; emailSentTo?: string;
}

// --- Utility ---
const sanitizeInput = (input: string): string => input?.replace(/[<>{}]/g, "").replace(/\n+/g, " ").trim() || "";
...
} else { logger.warn("Cloudinary credentials are not fully configured. Uploads will likely fail."); }

async function uploadPdfToCloudinary(localFilePath: string, fileName: string, operationId: string): Promise<string> {
...
async function logPdfQuoteToDb(logData: QuoteLogInputData): Promise<void> {
  try {
    const payloadForDb: Partial<IQuoteLog> & { totalAmount: number } = {
    ...
    const newLog = new QuoteLog(payloadForDb);
    await newLog.save();
...
async function generatePdfQuoteToolLogic(
  toolInput: z.infer<typeof flexiblePdfQuoteSchema>, customerName: string, business: BusinessProfile,
  operationId: string, customerId: string
): Promise<string> {
  ...
  const baseLogData: QuoteLogInputData = {
  ...
  for (const item of lineItems) {
    ...
    if (item.unitPrice !== undefined && qty > 0) { itemTotal = qty * item.unitPrice; }
    else if (item.totalPrice === undefined && item.unitPrice !== undefined) {
      itemTotal = qty * (item.unitPrice || 0);
    }
    else if (item.totalPrice !== undefined && item.unitPrice === undefined && qty > 0 && item.totalPrice > 0) { unitP = item.totalPrice / qty; }
    else if (item.totalPrice === undefined && item.unitPrice === undefined) {
  ...
  for (const variantKey in robotoPaths) {
    const variant = variantKey as keyof typeof robotoPaths;
    if (!fs.existsSync(robotoPaths[variant])) {
  ...
  const printer = new PdfPrinter({ Roboto: robotoPaths });
  ...
  try {
    ...
    const quoteOutputDir = path.join(projectRoot, 'quotes');
    if (!fs.existsSync(quoteOutputDir)) { fs.mkdirSync(quoteOutputDir, { recursive: true }); }
    const safeCustomerName = sanitizeFilename(customerName); const safeQuoteTitle = sanitizeFilename(quoteTitle || "Quote");
    fileName = `${safeQuoteTitle}_${safeCustomerName}_${Date.now()}.pdf`;
    filePath = path.join(quoteOutputDir, fileName);
    (baseLogData as QuoteLogInputData).pdfLocalPath = filePath;
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      pdfDoc.pipe(stream); pdfDoc.end();
    });
    logger.info({ operationId }, "Local PDF save stream promise resolved.");
    await logPdfQuoteToDb({ ...(baseLogData as QuoteLogInputData), status: 'generated_local' });
    let cloudinaryUrl = '';
    try {
      cloudinaryUrl = await uploadPdfToCloudinary(filePath, fileName, operationId);
      await logPdfQuoteToDb({ ...(baseLogData as QuoteLogInputData), pdfCloudinaryUrl: cloudinaryUrl, status: 'uploaded_cloud' });
      ...
      logger.info({ operationId, returnedMessage: successMsg }, "PDF generated and uploaded successfully."); return successMsg;
    } catch (uploadError: any) {
      logger.error({ ...baseLogData, filePath, error: uploadError.message }, "Cloudinary upload failed.");
      await logPdfQuoteToDb({ ...(baseLogData as QuoteLogInputData), status: 'failed_upload', errorMessage: `Cloudinary upload: ${uploadError.message}` });
    ...
    await logPdfQuoteToDb({ ...(baseLogData as QuoteLogInputData), status: 'failed_generation', errorMessage: `PDF save process: ${error.message}` });
    return errorMsg;
  }
}

// --- Zod Schemas for Tools ---
const flexiblePdfQuoteSchema = z.object({
  lineItems: z.array(
    z.object({
      description: z.string().describe("Detailed description of the product or service."),
      quantity: z.number().positive().optional().default(1).describe("Quantity. Defaults to 1 if not applicable/specified."),
      unit: z.string().optional().describe("Unit of measure (e.g., 'kg', 'hour', 'item', 'service')."),
      unitPrice: z.number().nonnegative().optional().describe("Price per unit."),
      totalPrice: z.number().nonnegative().describe("Total price for this line item. This is primary if unitPrice is missing for a service."),
      notes: z.string().optional().describe("Any specific notes for this line item."),
    })
  ).min(1).describe("An array of at least one line item for the quote."),
  customerNotes: z.string().optional().describe("Overall notes for the customer on the quote."),
  quoteTitle: z.string().optional().default("Service Quote").describe("A title for the quote document."),
});

const sendEmailWithLinkSchema = z.object({
  recipientEmail: z.string().email().describe("The customer's email address."),
  subject: z.string().describe("The email subject."),
  bodyText: z.string().describe("The main email content. You will compose this. If a PDF URL is available, you can mention it, but the URL will also be appended separately by the system if provided in pdfQuoteUrl."),
  pdfQuoteUrl: z.string().url().optional().describe("CRITICAL: The EXACT, complete, and valid Cloudinary URL of the PDF generated in the previous step. Do NOT use placeholders."),
});


// --- Tool Classes ---
class PdfQuoteTool extends StructuredTool<typeof flexiblePdfQuoteSchema> {
  schema = flexiblePdfQuoteSchema; name = "create_flexible_pdf_quote"; description: string;
  private customerName: string; private customerId: string; private business: BusinessProfile;
  private operationId: string;
  constructor(customerName: string, customerId: string, business: BusinessProfile, operationId: string) {
    super(); this.customerName = customerName; this.customerId = customerId; this.business = business;
  ...
  protected async _call(arg: z.infer<typeof flexiblePdfQuoteSchema>): Promise<string> {
    if (!arg.lineItems || arg.lineItems.length === 0) { return "ERROR_INVALID_INPUT: No line items were provided for the quote."; }
    try {
      return await generatePdfQuoteToolLogic(arg, this.customerName, this.business, this.operationId, this.customerId);
    } catch (toolLogicError: any) {
...
async function sendEmailWithLinkToolLogic(toolInput: z.infer<typeof sendEmailWithLinkSchema>, operationId: string, customerId: string, customerName: string, businessId: string): Promise<string> {
  ...
  const GMAIL_USER = process.env.GMAIL_USER; const GMAIL_APP_PASSWORD = process.env.GMAIL_APP_PASSWORD;
  if (!GMAIL_USER || !GMAIL_APP_PASSWORD) {
    const errMsg = "Email sending is not configured: Gmail credentials missing.";
    logger.error({ operationId }, errMsg); await logPdfQuoteToDb({ ...baseLogData, errorMessage: errMsg });
...
class EmailQuoteLinkTool extends StructuredTool<typeof sendEmailWithLinkSchema> {
  schema = sendEmailWithLinkSchema; name = "send_quote_link_via_email";
  description: string; // Description will be set in the constructor or main function
  private operationId: string; private customerId: string; private customerName: string; private businessId: string;

  constructor(operationId: string, customerId: string, customerName: string, businessId: string, businessCloudinaryName?: string) {
    super(); this.operationId = operationId; this.customerId = customerId;
    this.customerName = customerName; this.businessId = businessId;
  ...
  protected async _call(arg: z.infer<typeof sendEmailWithLinkSchema>): Promise<string> {
...
export const generateAnswerFromOpenAI = async (
  query: string, context: string, parameters: Parameters, business: BusinessProfile,
  aiAgentProfile: AiAgentProfile, customerId: string, customerName: string,
  chatHistoryFromDb: ChatMessage[] = []
): Promise<string> => {
  ...
  if (!businessId) {
    ...
    const logData: QuoteLogInputData = { customerId, customerName, businessId: "UNKNOWN", quoteTitle: "N/A - Missing Business ID", lineItems: [], grandTotal: 0, status: 'failed_generation', errorMessage: "Missing business ID", operationId };
  ...
  try {
    const cached = await redisClient.get(replyCacheKey);
    if (cached) {
  ...
  let longTermInteractionMemory: string | undefined;
  let customerProfileMemory: string | undefined;
  try {
    [longTermInteractionMemory, customerProfileMemory] = await Promise.all([
      fetchContextMemory(sanitizedQuery, businessId, customerId),
      getMessageMemory(businessId, customerId, sanitizedQuery),
    ]);
  } catch (err: any) {
  ...
  let combinedChatHistory: BaseMessage[] = [];
  let rawRedisHistory: ChatMessage[] = [];
  try {
    const prevRedis = await redisClient.get(chatKey);
    if (prevRedis) {
      rawRedisHistory = JSON.parse(prevRedis);
      rawRedisHistory.forEach(msg => {
        if (msg.role === "user") combinedChatHistory.push(new HumanMessage(sanitizeInput(msg.content)));
        else if (msg.role === "assistant") {
          let content = msg.content;
          if (typeof content !== 'string') { content = JSON.stringify(content); }
          combinedChatHistory.push(new AIMessage(sanitizeInput(content)));
        }
      });
    }
  } catch (err: any) {
  ...
  dbHistoryArray.forEach(msg => {
    ...
    if (role === "user") combinedChatHistory.unshift(new HumanMessage(content));
    else if (role === "assistant") combinedChatHistory.unshift(new AIMessage(content));
  });
...
`.trim();
  ...
  let agentResponseOutput: string = "I’m sorry, something went wrong. Please try again.";
...
// @ts-ignore
if (!global.redisClient && !(typeof redisClient !== 'undefined' && redisClient && typeof redisClient.get === 'function')) {
  console.warn("Using MOCK redisClient. Implement in ./config/redis.ts.");
  // @ts-ignore
  global.redisClient = {
    get: async (key: string): Promise<string | null> => { logger.debug({ key }, "MOCK redisClient.get"); return null; },
    set: async (key: string, value: string, options?: any): Promise<string | null> => { logger.debug({ key, value, options }, "MOCK redisClient.set"); return "OK"; },
  };
  // @ts-ignore
  if (typeof redisClient === 'undefined' || (redisClient && typeof redisClient.get !== 'function')) {
    // @ts-ignore
...
// @ts-ignore
const _mockFetchContextMemory = async (query: string, businessId: string, customerId: string): Promise<string> => {
  logger.debug({ query, businessId, customerId }, "MOCK fetchContextMemory");
  return "Mocked long-term interaction memory.";
};
// @ts-ignore
const _mockGetMessageMemory = async (businessId: string, customerId: string, query: string): Promise<string> => {
...
const _fetchContextMemory = typeof fetchContextMemory === 'function' && Object.keys(fetchContextMemory).length > 0 ? fetchContextMemory : _mockFetchContextMemory;
const _getMessageMemory = typeof getMessageMemory === 'function' && Object.keys(getMessageMemory).length > 0 ? getMessageMemory : _mockGetMessageMemory;
